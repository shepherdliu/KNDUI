...
此控件的基础用法用于显示、编辑某一变量或系统参数的值，可根据需要设置控件的读写模式，线框格式等属性。
choices参数是对data控件基础功能的扩展，只读模式下，可以用此控件显示一些字符串。例如，根据某一变量的值来显示此时的工作坐标系？？？？？？？。确认模式下，可以用此控件显示一个下拉文本框，选择下拉框中的"值"会将其对应的"键"值赋给指定的变量。
action参数使操作此控件时机床能执行设定的"动作"。例如，
